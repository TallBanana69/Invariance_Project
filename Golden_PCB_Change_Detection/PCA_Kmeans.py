...
from skimage.transform import resize as imresize  # For imresize alternative
...
def get_descriptors(image1, image2, window_size, pca_dim_gray, pca_dim_rgb):
    ...
    diff_image = cv2.absdiff(image1, image2)  # Calculate absolute difference
    diff_image = color.rgb2gray(diff_image)  # Convert to grayscale (NumPy array with float values)

    # Scale and convert NumPy array to 8-bit for saving
    diff_image_uint8 = (np.clip(diff_image, 0, 1) * 255).astype(np.uint8)

    # Save using PIL
    imsave(global_variables.output_dir + '/diff.jpg', diff_image_uint8)

    # Pad image
    diff_image_padded = np.pad(diff_image_uint8, 
                               ((window_size // 2, window_size // 2), (window_size // 2, window_size // 2)), 
                               'constant')

    for i in range(image1.shape[0]):
        for j in range(image1.shape[1]):
            descriptors[i, j, :] = diff_image_padded[i:i + window_size, j:j + window_size].ravel()
    ...
    # Pad images
    diff_image_r = np.pad(diff_image_r, ((window_size // 2, window_size // 2), (window_size // 2, window_size // 2)), 'constant')
    diff_image_g = np.pad(diff_image_g, ((window_size // 2, window_size // 2), (window_size // 2, window_size // 2)), 'constant')
    diff_image_b = np.pad(diff_image_b, ((window_size // 2, window_size // 2), (window_size // 2, window_size // 2)), 'constant')
